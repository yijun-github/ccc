...
*/}
export default function ScatterwarElectPartdiff({ data }) {
  ...
  function formatData(suburbData) {
    ...
    suburbData['features'].forEach(suburb => {
      if ((suburb['properties']['SAL_NAME21'] != undefined) && (suburb['properties']['war_neg%'] != undefined) && 
        (suburb['properties']['male_female_part_diff'] != undefined) && (suburb['properties']['war_total'] >= 5)) {
        
        var name = suburb['properties']['SAL_NAME21'];
        var neg_perc = round(suburb['properties']['war_neg%']*100, 0);
        var m_f_diff = round(suburb['properties']['male_female_part_diff']*100, 0);
        if (suburb['properties']['labor%'] >= 0.5) {
          labor.push({'suburb': name, 'war_neg%': neg_perc, 'male_female_part_diff': m_f_diff});
        } else if (suburb['properties']['labor%'] < 0.5) {
          lib.push({'suburb': name, 'war_neg%': neg_perc, 'male_female_part_diff': m_f_diff});
        }
      }
    })
  ...
  return (
    <>
    <ChartTitle title="Male-Female Emp % diff vs Negative Sentiment % (Suburbs)" />
    <ScatterChart
      margin={{
        top: 20,
        right: 20,
    ...
    >
      ...
      <YAxis type="number" dataKey="war_neg%" name="Neg. Sent. %"
        label={{ value: 'Neg. Sent. %', angle: -90, position: 'insideLeft' }}
        />
